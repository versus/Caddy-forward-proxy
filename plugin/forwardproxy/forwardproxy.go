...
import (
	...
	"figleafproxy/plugin/forwardproxy/httpclient"
...
func (fp *ForwardProxy) checkCredentials(r *http.Request) error {
	...
	login, pass, err := fp.decodeAuthHeader(pa)
	if err != nil {
		return err
	}

	realPass, err := fp.credentialsRepository.GetPassword(login)
	if err != nil || realPass != pass {
...
func (fp *ForwardProxy) decodeAuthHeader(header string) (string, string, error) {
	pa := strings.Split(header, " ")
	if len(pa) != 2 {
	...
	dec, err := base64.StdEncoding.DecodeString(pa[1])
	if err != nil {
		return "", "", errors.New("Cannot decode credentials")
	}
...
func (fp *ForwardProxy) ServeHTTP(w http.ResponseWriter, r *http.Request) (int, error) {
	...
	} else { // TODO: investigate this bug and fix properly
		// Hardcode fix for 502 error and hungreds of headers in response
		if r.URL.Path == "/ping" || r.URL.Path == "/ping/" {
			return http.StatusOK, authErr
