...
import (
	...
	"crypto/tls"
	"errors"
	fpd "figleafproxy/plugin/forwardproxy/dynamo"
	"fmt"
...
func setup(c *caddy.Controller) error {
	...
	if cfg.Region == "" {
		r, err := ec2metadata.New(cfg).Region()
		if err != nil {
			cfg.Region = "us-east"
		}
	...
	for c.NextBlock() {
		...
		args := c.RemainingArgs()
		switch subdirective {
		case "credentials_table":
			if len(args) != 1 {
				return c.ArgErr()
			}
			if len(args[0]) == 0 {
				return c.Err("empty credentials table name is not allowed")
			}

			credentialsRepo := fpd.NewCredentialsRepository(dynamo, args[0])
			fp.authRequired = true
			fp.credentialsRepository = credentialsRepo
		case "ports":
			if len(args) == 0 {
				return c.ArgErr()
			}
		...
		case "acl":
			...
			for {
				if !c.Next() {
					return c.Err("acl block must be ended by closing curly braces \"}\"")
				}
				aclDirective := c.Val()
				args := c.RemainingArgs()
				if aclDirective == "}" {
